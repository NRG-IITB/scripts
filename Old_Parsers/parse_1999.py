...
# ===============================================================
#   SUMMARY PARSER (1999)  — matches the PDF structure shown
# ===============================================================
def parse_summary_1999(path):
    data = []
    ...
    with pdfplumber.open(path) as pdf:
        for page in pdf.pages:
            ...
            # ------------------------------------------------------
            # ELECTORS — matches your screenshot
            # ------------------------------------------------------
            eg = re.findall(r"GENERAL\s+(\d+)\s+(\d+)\s+(\d+)", text)
            if eg:
                m, w, t = eg[0]
                entry["Electors"]["General"] = {
                    "Men": safe_int(m), "Women": safe_int(w),
                    "Third_Gender": 0, "Total": safe_int(t)
                }
            ...
            # ------------------------------------------------------
            # VOTERS WHO VOTED (GENERAL + POSTAL)
            # ------------------------------------------------------
            vg = re.findall(r"III\.\s*ELECTORS WHO VOTED.*?GENERAL\s+(\d+)\s+(\d+)\s+(\d+)", text, re.S)
            if vg:
                m, w, t = vg[0]
            ...
            pp = re.findall(r"POLL PERCENTAGE\s*:\s*([\d\.]+)", text)
            if pp:
                entry["Voters"]["POLLING PERCENTAGE"]["Total"] = safe_float(pp[0])

            # ------------------------------------------------------
            # VOTES (POLLED, VALID, REJECTED, MISSING, TENDERED)
            # ------------------------------------------------------
            ...
            # ------------------------------------------------------
            # DATES (Polling, Counting, Declaration)
            # ------------------------------------------------------
...
# ===============================================================
#   DETAILED PARSER (1999)
# ===============================================================
def parse_detailed_1999(path):
    candidates = {}

    # Regex: 1999 format (NO AGE, NO CAT, but SEX + PARTY + VOTES + %)
    cand_re = re.compile(
    ...
    with pdfplumber.open(path) as pdf:
        for page in pdf.pages:
            ...
            # Find constituency heads
            heads = re.findall(r"Constituency\s+(\d+)\s+([A-Za-z\s\(\)]+)", text)
            if not heads:
                continue
            ...
            for line in text.split("\n"):
                ...
                if m:
                    ...
                    # Assign to last constituency
                    cid = f"S01-{heads[-1][0]}"
                    candidates[cid].append(cand)
...
# ===============================================================
def merge_1999(summary, detailed):
    final = []
    ...
    for s in summary:
        ...
        if cid in detailed:
            ...
            total_elect = s["Electors"]["Total"]["Total"]

            for c in s["Candidates"]:
                c["Valid Votes"] = total_valid
                c["Total Electors"] = total_elect

                if total_valid > 0:
                    tv = c["Votes Secured"]["Total"]
                    c["Over Total Valid Votes Polled In Constituency"] = round(tv * 100 / total_valid, 2)
...
#   MAIN
# ===============================================================
summary_path = "/mnt/d/IIT_B_Project/SL_NON GIT/pdf_before 2009/summary/1999_summary_trimmed.pdf"
detailed_path = "/mnt/d/IIT_B_Project/SL_NON GIT/pdf_before 2009/detailed/1999_detailed.pdf"
output_path = "/mnt/d/IIT_B_Project/SL_NON GIT/parsed_new/parsed_new_1999.json"

summary = parse_summary_1999(summary_path)
detailed = parse_detailed_1999(detailed_path)
merged = merge_1999(summary, detailed)
